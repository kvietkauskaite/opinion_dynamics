...
sampled from the uniform distribution and fixed radicals included.
The calculated data is first saved in a csv file and then results
are plotted in a bar plot.
"""

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.pyplot import figure
from functions.HK_discrete_r import HK_discrete_r
...
for r in R:
    ...
    for n in N:
        ...
        for t in range(total_runs):
            ...
            # SIMULATION
            x = HK_discrete_r(r, n, x0, n_rad, x0_rad, stop, 'LAST_STEP', include_self = False)
...
# save the results
np.savetxt('discrete analysis/HK 3 results_radicals.txt', clusters, fmt='%d')

# write variance CSV file
with open("discrete analysis/HK 3 results_radicals (var).csv","w") as f:
...
with open("discrete_analysis/HK 3 results_radicals (var).csv", "r") as f:
...
# load the results
clusters = np.loadtxt('discrete_analysis/HK 3 results_radicals.txt', dtype=int)
...
for r in R:
    ...
    for n in N:
        ...
        # SIMULATION
        rx = HK_discrete_r(r, n, rx0, n_rad, x0_rad, stop, 'LAST_STEP', include_self = False)
